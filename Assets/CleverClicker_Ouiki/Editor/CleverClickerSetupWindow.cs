...
{
    public class CleverClickerSetupWindow : EditorWindow
    {
        ...
        [MenuItem("Tools/Clever Clicker/Setup", false, 0)]
        public static void ShowWindow()
        {
            CleverClickerSetupWindow window = GetWindow<CleverClickerSetupWindow>("Clever Clicker Setup");
        ...
        {
            bool isActive = EditorPrefs.GetString("CleverClicker_LastPreset", "Standard") == id;
            if (isActive) GUI.backgroundColor = activeColor;
            
            if (GUILayout.Button(label, _presetButtonStyle, GUILayout.Height(55)))
            {
                CleverClickerSettings.ApplyPreset(id);
                EditorPrefs.SetString("CleverClicker_LastPreset", id);
            }
        ...
        private void OnGUI()
        {
            ...
            GUILayout.Label("CLEVER CLICKER", _titleStyle);
            GUILayout.Label("Ouiki Premium Edition", EditorStyles.centeredGreyMiniLabel);
            ...
            if (_isRecording)
            {
                ...
                if (evt.isKey || evt.isMouse)
                {
                    ...
                    {
                        CleverClickerSettings.ModifierKeys = evt.modifiers;
                        EditorPrefs.SetString("CleverClicker_LastPreset", "Custom");
                        Repaint();
                    }
                }
            }
            ...
            EditorGUILayout.BeginHorizontal();
            CleverClickerSettings.ShowOnStartup = EditorGUILayout.ToggleLeft("Show this window on startup", CleverClickerSettings.ShowOnStartup);
            if (GUILayout.Button("Reset", GUILayout.Width(60))) CleverClickerSettings.ResetToDefaults();
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.Space(10);
            if (GUILayout.Button("SAVE & FINISH", GUILayout.Height(45)))
            {
                CleverClickerSettings.IsFirstRun = false;
                Close();
            }
        ...
        {
            bool isActive = (CleverClickerSettings.ModifierKeys & modifier) != 0;
            ...
            {
                if (isActive) 
                    CleverClickerSettings.ModifierKeys &= ~modifier;
                else 
                    CleverClickerSettings.ModifierKeys |= modifier;

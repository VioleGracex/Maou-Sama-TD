...
{
    [InitializeOnLoad]
    public static class SwiftSelectManager
    {
        private static GameObject _hoveredObject;

        static SwiftSelectManager()
        {
            SceneView.duringSceneGui += OnSceneGUI;
            EditorApplication.delayCall += () => {
                if (SwiftSelectSettings.IsFirstRun || SwiftSelectSettings.ShowOnStartup)
                    SwiftSelectSetupWindow.ShowWindow();
            };
        }
        ...
        private static void OnSceneGUI(SceneView sceneView)
        {
            ...
            if (e.type == EventType.MouseDown && (e.button == 0 || e.button == 1) && (e.modifiers == SwiftSelectSettings.ModifierKeys))
            {
                ...
                var validObjects = pickedObjects
                    .OfType<GameObject>()
                    .Where(obj => (SwiftSelectSettings.ExcludeLayers & (1 << obj.layer)) == 0)
                    .Distinct()
                    .ToList();
                ...
                if (validObjects.Count > 0)
                {
                    ...
                    SwiftSelectPopup.ShowPopup(screenPos, validObjects, e.button == 1);
                    e.Use();
                }
            }
            ...
            {
                DrawHighlight(_hoveredObject, SwiftSelectSettings.HighlightColor);
            }
        }

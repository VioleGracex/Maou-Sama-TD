...
```text
[SUBJECT_DESCRIPTION], premium 2.5D game item icon, dark fantasy anime style, hand-painted digital illustration, thick dark bronze metallic trim, subtle magical light emission, vibrant saturated colors, fine sharp details, game-ready icon asset, isolated on a solid dark gray background, volumetric shading, cinematic rim lighting, 8k resolution
```

### Key Style Rules:
- **Background**: Always request a `solid dark gray background` or `solid black background`. This makes alpha channel masking and background isolation (removing the background to make it transparent) incredibly clean in Photoshop or online tools.
...
## 🛠️ Sprite Extraction & Unity Configuration Guide

Once you have generated your PNG assets using these prompts, follow these steps to import and optimize them for the **Vault Inventory UI**:

### 1. Transparency Extraction (Background Removal)
If your generator outputs a solid dark gray or black background:
- Use an image editing tool (e.g. Photoshop, Photopea, or an AI background remover) to select the solid dark background and delete it.
- **Photoshop Tip**: Use the *Magic Wand Tool* with a tolerance of `10-15` and *Anti-alias* checked. Select the background, press delete, and export as a transparent **PNG-24**.

-- Scenario 5: Replay Level 1 (Free Play)
-- Flow: connect/launch game (no clear data) → navigate Conquest map → select Level 1 node → Engage → Start Mission → Start Battle → Deploy Ignis → Ultimate → Level 1 victory.
...
local function run_tests()
    ...
    -- ============================================================
    -- STAGE 3: Select Level 1 Node
    -- ============================================================
    set_stage("3. Select Node 1")
    log_test("Select Node", "STARTING", "Selecting Level 1 node on map...")

    local node1 = wait_template("node_level_1", 10)
    if node1 then
        click(node1.x, node1.y)
        log_test("Select Node", "PASS", "Level 1 Node selected.")
    else
        click(200, 240) -- approximate node 1 coordinates
    ...
    -- Check if tutorial choice prompt appears and bypass it to play by myself
    local play_myself_btn = wait_template("PlayMyself_Btn", 4)
    if not play_myself_btn then
        play_myself_btn = wait_template("NoButton", 1)
    end
    ...
    if next_lvl_btn then
        ...
        log_test("Victory", "PASS", "Level 1 cleared and returned.")
    else
